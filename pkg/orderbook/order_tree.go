...
import (
	"github.com/Xib1uvXi/MatchingEngine/pkg/orderbook/rbt"
	"strconv"
...
type OrderTree struct {
	priceTree *rbt.RedBlackTreeExtended
	// Dictionary containing price : OrderList object
	priceMap map[string]*OrderList
	// Dictionary containing order_id : Order object
	orderMap map[string]*Order
	// Contains total quantity from all Orders in tree
	volume decimal.Decimal
	// Contains count of Orders in tree
	numOrders int
	// Number of different prices in tree (http://en.wikipedia.org/wiki/Order_book_(trading)#Book_depth)
	depth int
}

func NewOrderTree() *OrderTree {
	priceTree := &rbt.RedBlackTreeExtended{Tree: redblacktree.NewWith(decimalComparator)}
